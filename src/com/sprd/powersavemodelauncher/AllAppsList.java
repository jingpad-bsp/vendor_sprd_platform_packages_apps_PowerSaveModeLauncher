...
class AllAppsList {
    ...
    public void add(AppInfo info) {
        if (findActivity(data, info.componentName, info.user)
                || !isAllowedAdd(info.componentName.getPackageName(),info.user)) {
            return;
        }
        data.add(info);
        added.add(info);
    ...
    private boolean isAllowedAdd(String pkgName, UserHandleCompat user) {
        return !LauncherModel.mRemovedPackageList.contains(pkgName);
    }
     ...
     */
    public void updatePackageFlags(StringFilter pkgFilter, UserHandleCompat user, boolean isDisabled) {
        final List<AppInfo> data = this.data;
        for (int i = data.size() - 1; i >= 0; i--) {
            AppInfo info = data.get(i);
            final ComponentName component = info.intent.getComponent();
            if (info.user.equals(user) && pkgFilter.matches(component.getPackageName())) {
                info.isDisabled = isDisabled ? 1:0;
                modified.add(info);
            }
        }
    }
