...
public class IconNormalizer {
    ...
    private IconNormalizer(Context context) {
        ...
        mMaxSize = LauncherAppState.getInstance(context).getInvariantDeviceProfile().iconBitmapSize * 2;
    ...
    public synchronized void getVisibleAreaBounds(@NonNull Drawable d, @Nullable Rect outBounds) {
        int width = d.getIntrinsicWidth();
        int height = d.getIntrinsicHeight();
        if (width <= 0 || height <= 0) {
            width = width <= 0 || width > mMaxSize ? mMaxSize : width;
        ...
        if (outBounds != null) {
            outBounds.set(leftX, topY, rightX, bottomY);
        }
    }
    ...
    public static boolean isExternalStorageAvailabled(){
        return Environment.getExternalStorageState() == Environment.MEDIA_MOUNTED;
    }

    private static boolean isDebugBitmapARGB(){
        return DEBUG && isExternalStorageAvailabled();
    }
}
